...
OOIWidth=np.minimum( (xdims[1] - xdims[0]), (ydims[1]-ydims[0] ) ) /10 # beacon representing the person is drawn as circle in the map(ellipse indeed, but looks like a circle due to adjustments)
OOIHeight=OOIWidth    
...
def processFunction():
    ...
    while True:
        initialPositionOfThePerson=np.random.uniform(low=[xmin,ymin], high=[max(xmin,xmax-1),max(ymin,ymax-1)], size=(2))
        isCollision=False
