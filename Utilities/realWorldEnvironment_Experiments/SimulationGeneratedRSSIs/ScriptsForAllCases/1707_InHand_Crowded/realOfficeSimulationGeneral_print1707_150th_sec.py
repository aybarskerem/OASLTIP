...
print "TESTS FOR 17:07 is starting..."
with open('AllValues1707_salt.csv') as csvfile:
...
def animate(iterNo, ax, macID, currPerson, NumberOfParticles,  xdims=(0, 50), ydims=(0, 50), maxSignalError=20,  movingLimit=2, pastCoeff=0, minUsefulSignal=-90, 
    ...
    '''if iterNo == 149:
        ...
        #plt.savefig('In-Hand_Crowded.png')
        #sys.exit()
    
    if iterNo >=0 and iterNo <50:
        global realIndex
        if isProcessed:
            my_acc = np.linalg.norm(currPerson.mu-currPerson.beacon_pos) 
            with open(os.path.join(path_of_this_script, "1707_mu.txt"),"a+") as myOutFile:	
