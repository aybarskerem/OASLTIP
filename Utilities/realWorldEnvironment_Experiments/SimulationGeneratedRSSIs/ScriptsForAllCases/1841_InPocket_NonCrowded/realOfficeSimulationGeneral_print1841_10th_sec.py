...
print "TESTS FOR 18:41 is starting..."
with open('AllValues1841_salt.csv') as csvfile:
    ...
    def calc_RSSIs_to_Receivers(self,minSignalValue,minUsefulSignal,maxSignalError):
        ...
        for receiverPosition in self.receiverPositions:
            ...
            for blockIndex, blockPosition in enumerate(blockPositions):
                receiverBeaconBlockIntersection=findRectangleLineSegmentIntersectionPoints(receiverPosition,self.beacon_pos,blockPosition[blockIndex],blockWidth,blockLength)
            ...
            for roomIndex, roomPosition in enumerate(roomPositions):
                receiverBeaconRoomIntersection=findRectangleLineSegmentIntersectionPoints(receiverPosition,self.beacon_pos,roomPosition,roomWidth[roomIndex],roomLength[roomIndex])
            ...
            if self.mu is not None: #!= degildir kulanma cunku, array oldugu zaman burasi hata verir
                ...
                for roomIndex, roomPosition in enumerate(roomPositions):
                    receiverMeanRoomIntersection = findRectangleLineSegmentIntersectionPoints(receiverPosition,self.mu,roomPosition,roomWidth[roomIndex],roomLength[roomIndex])
...
def animate(iterNo, ax, macID, currPerson, NumberOfParticles,  xdims=(0, 50), ydims=(0, 50), maxSignalError=20,  movingLimit=2, pastCoeff=0, minUsefulSignal=-90, 
    ...
    if iterNo>=54:
        print "numberOfNotFounds is: ", numberOfNotFounds
    ...
    '''if iterNo == 9:
        ...
        #plt.savefig('In-Pocket_Non-Crowded.png')
        #sys.exit()

    if iterNo >=4 and iterNo <54:
        global realIndex
        if isProcessed:
            my_acc = np.linalg.norm(currPerson.mu-currPerson.beacon_pos) 
            with open(os.path.join(path_of_this_script, "1841_mu.txt"),"a+") as myOutFile:            	
