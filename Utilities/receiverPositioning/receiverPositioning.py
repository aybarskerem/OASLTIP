...
if len(sys.argv)!=3:
    print "run as 'python <script name> <numberOfReceivers> <output file name>'"
    exit(1)


numberOfReceivers=int(sys.argv[1])
...
roomWallWidth=roomWidth[0] * WallRoomRatio # express line witdht in terms of data points instead of axis 
...
def main():
    ...
    #roomPositions=[[7.5,9.5],[7,3.5]]
    #blockPositions=[[5,3],[9,8]]
...
def animate(iterNo, ax, macID, currPerson, NumberOfParticles,  xdims=(0, 50), ydims=(0, 50), maxSignalError=20,  movingLimit=2, pastCoeff=0, minValidSignal=-90, 
    ...
    ax.text(0.2,1.03,s=r"\textbf{CASE: \# of RECEIVERS = " + str(numberOfReceivers)  + "}"
       , horizontalalignment='left' , verticalalignment='bottom' , fontsize=20, transform=ax.transAxes )

    #ax.text(0.4,1.03,s="CASE: # of RECEIVERS = " + str(numberOfReceivers) + "\n Current Accuracy is: " + str(float("{0:.2f}".format(CurrAccuracy))) + " m"
